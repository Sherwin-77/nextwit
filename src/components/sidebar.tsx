...
export default function SideBar() {
  ...
  return (
    <div className="flex flex-col bg-sky-200 dark:bg-gray-800 rounded-xl overflow-hidden">
      ...
      {suggestions ? (
        suggestions.map((obj) => (
          <Link href={`/profile/${obj.username}`} key={obj._id}>
            <div className="flex flex-col hover:bg-neutral-100 dark:hover:bg-gray-900 p-3 hover:cursor-pointer">
              ...
              <span className="line-clamp-1 whitespace-pre-wrap text-slate-500">
                {obj.bio ? obj.bio : "No description yet"}
              </span>
            </div>
          </Link>
        ))
      ) : (
        <div className="p-3">{errorMessage ?? <Loading />}</div>
      )}
    </div>
  );
}
